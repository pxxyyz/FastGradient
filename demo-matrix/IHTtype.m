...
if (~isempty(opts))
    ...
    Theta = opts.Theta;
    r = opts.r;
    if isfield(opts,'Problem');Problem = opts.Problem;end
...
for iter=1:MaxIter
    ...
    if UseNAG && eta(iter)>0
        if strcmp(Geometric, 'Euclidean')
            [U,~,V] = svds(Y, r);
        elseif strcmp(Geometric, 'Riemannian') && strcmp(Retract, 'Orth')
            [Y, UY, VY] = OrthRetract(Y, U, V);
        end
    else
        UY = U; VY = V;
    end
    G = gradient(Y, Xob, Problem, opts);
...
function G = gradient(X, Xob, Problem, opts)
G=zeros(size(X));
if strcmp(Problem, 'MC')
    G(opts.S)=X(opts.S)-Xob(opts.S);
elseif strcmp(Problem, 'MS')
    for l = 1:length(Xob)
